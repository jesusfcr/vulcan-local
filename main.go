...
func main() {
	...
	var help bool
	var configFile, assetOptions string
	...
	flag.StringVar(&assetOptions, "o", "", `Options related to the asset (-t) used in all the their checks (i.e. '{"depth":"1", "max_scan_duration": 1}' )`)
	...
	if assetOptions != "" {
		...
		if err = json.Unmarshal([]byte(assetOptions), &cfg.Asset.Options); err != nil {
